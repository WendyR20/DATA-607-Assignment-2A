...
When we used glimpse() we saw that there were null values in the dataframe, let's find out more.

```{r null values}
sapply(df_movies, function(x) sum(is.na(x)))
sapply(df_takers, function(x) sum(is.na(x)))
...
Now let us delete the rows wherein the survey taker did not see a movie, i.e. when the rating and rating_description columns are empty. 

```{r null values cleaned}
df_ratings_clean <- df_ratings %>%
  drop_na(rating,rating_description)
